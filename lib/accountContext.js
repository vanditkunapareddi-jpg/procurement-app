...
export function AccountProvider({ children }) {
  ...
  useEffect(() => {
    if (!ready || accountId || !auth) return undefined;
    let cancelled = false;
    const unsub = auth.onAuthStateChanged(async (user) => {
      if (!user || cancelled || provisioningRef.current) return;
      provisioningRef.current = true;
      try {
        const { accountId: newId } = await ensureAccountForUser(user.uid, {
  ...
  const provisionAndSetAccount = async (uid, opts = {}) => {
    const { accountId: newId } = await ensureAccountForUser(uid, {
      ...opts,
      accountId,
    });
    updateAccount(newId);
